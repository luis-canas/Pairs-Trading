...
#         # lower bound and upper bound for pattern_distances
#         variables_lb = [0,1,1,1,0]
#         variables_ub = [50,MAX_SIZE,MAX_SIZE,DAYS_CLOSE,1]
...
#         # extract chromossomes
#         long_genes = x[:self.ENTER_LONG]
#         dist_long,word_size_long ,window_size_long,days_long,measure_type_l,pattern_long = long_genes[0],round(long_genes[1]),round(long_genes[2]), round(long_genes[3]),round(long_genes[4]),np.round(long_genes[5:])
#         pattern_long=pattern_long[:word_size_long]

#         exit_long_genes = x[self.ENTER_LONG:self.EXIT_LONG]
#         dist_exit_long, word_size_exit_long ,window_size_exit_long,measure_type_el,pattern_exit_long = exit_long_genes[0], round(exit_long_genes[1]), round(exit_long_genes[2]), round(long_genes[4]),np.round(exit_long_genes[5:])
#         pattern_exit_long=pattern_exit_long[:word_size_exit_long]

#         short_genes = x[self.EXIT_LONG:self.ENTER_SHORT]
#         dist_short,word_size_short ,window_size_short, days_short,measure_type_s,pattern_short = short_genes[0], round(short_genes[1]),round(short_genes[2]),round(long_genes[3]),round(long_genes[4]),np.round(short_genes[5:])
#         pattern_short=pattern_short[:word_size_short]

#         exit_short_genes = x[self.ENTER_SHORT:self.EXIT_SHORT]
#         dist_exit_short, word_size_exit_short ,window_size_exit_short,measure_type_es,pattern_exit_short = exit_short_genes[0],round(exit_short_genes[1]),round(exit_short_genes[2]), round(long_genes[4]),np.round(exit_short_genes[5:])
#         pattern_exit_short=pattern_exit_short[:word_size_exit_short]
...
#                     if long_sax_seq is not None:
#                         l_dist = self._calc_pattern_dist(long_sax_seq, pattern_long,word_size_long,window_size_long,measure_type_l)
#                     if short_sax_seq is not None:
#                         s_dist = self._calc_pattern_dist(short_sax_seq, pattern_short,word_size_short,window_size_short,measure_type_s)
...
#                 elif position == LONG_SPREAD:
#                     if long_sax_seq is not None:
#                         l_dist = self._calc_pattern_dist(long_sax_seq, pattern_exit_long,word_size_exit_long,window_size_exit_long,measure_type_el)
#                     if l_dist > dist_exit_long or day_count>days_long:
#                         position,cash_in_hand[day],stocks_in_hand=self._trade_decision(CLOSE_POSITION,FIXED_VALUE, cash_in_hand[day],stocks_in_hand,self.c1[day],self.c2[day])
#                         l_dist=s_dist=np.inf
#                         day_count=0
...
#                 elif position == SHORT_SPREAD:
#                     if short_sax_seq is not None:
#                         s_dist = self._calc_pattern_dist(short_sax_seq, pattern_exit_short,word_size_exit_short,window_size_exit_short,measure_type_es)
#                     if s_dist > dist_exit_short or day_count>days_short:
#                         position,cash_in_hand[day],stocks_in_hand=self._trade_decision(CLOSE_POSITION,FIXED_VALUE, cash_in_hand[day],stocks_in_hand,self.c1[day],self.c2[day])
#                         l_dist=s_dist=np.inf
#                         day_count=0
...
#     def _calc_pattern_dist(self,string1,string2,word_size ,window_size,measure_type):

#         if measure_type == 0:
#             dist = pattern_distance(string1,string2)
#         if measure_type == 1:
#             dist = min_dist(string1,string2,self.alphabet_size,window_size/word_size)
...
#     def __init__(self, spread, c1, c2, window_size,alphabet_size,days_pos=252,FIXED_VALUE=1000,commission=0.08,  market_impact=0.2, short_loan=1):
...
#         # lower bound and upper bound for pattern_distances
#         variables_lb = [0,1,1,1]
#         variables_ub = [50,MAX_SIZE,MAX_SIZE,days_pos]
