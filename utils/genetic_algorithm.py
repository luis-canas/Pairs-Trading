...
class SaxObjectivesLS(ElementwiseProblem):
    ...
    def __init__(self, spread, c1, c2, window_size,alphabet_size,DAYS_CLOSE=252,objectives=["ROI","MDD","SR","FREQ"],FIXED_VALUE=1000,commission=0.08,  market_impact=0.2, short_loan=1):
        ...
        self.ENTER_SHORT = 2*CHROMOSSOME_SIZE
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if word_size_long<=window_size_long and word_size_short<=window_size_short:
            ...
            for day in range(1,len(self.spread)-1):
                ...
                long_sax_seq,short_sax_seq=self._get_patterns(position,self.spread[:day+1],self.alphabet_size,word_size_long ,window_size_long,word_size_short ,window_size_short)
    ...
    def _get_patterns(self,position,spread,alphabet,word_size_long ,window_size_long,word_size_short ,window_size_short):
...
class SaxObjectivesRange(ElementwiseProblem):
